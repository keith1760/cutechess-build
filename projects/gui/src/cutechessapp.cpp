...
bool CuteChessApplication::evalBarBlackAtBottom() const
{
	// Defaults to off, matching Cute Chess's historical appearance
	// (the eval bar always shows white at the bottom). User
	// configurable from the Settings dialog's General tab.
	QSettings s;
	return s.value("ui/eval_bar_black_at_bottom", false).toBool();
}

void CuteChessApplication::setEvalBarBlackAtBottom(bool blackAtBottom)
{
	QSettings s;
	s.setValue("ui/eval_bar_black_at_bottom", blackAtBottom);

	emit evalBarBlackAtBottomChanged(blackAtBottom);
}
