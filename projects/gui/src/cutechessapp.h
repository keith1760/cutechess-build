...
class CuteChessApplication : public QApplication
{
	...
	public:
		...
		/*!
		 * Records \a geometry, \a windowState and \a dockVisibility as
		 * the most recently known main window layout.
		 *
		 * \a dockVisibility maps each View-menu dock widget's
		 * objectName() to whether it was ticked/visible. It is saved
		 * and restored as its own set of plain booleans, independently
		 * of \a windowState, because QMainWindow::restoreState() bundles
		 * dock visibility together with dock position/layout into one
		 * opaque blob that it silently discards *in its entirety* if it
		 * doesn't exactly match the current set of dock widgets (for
		 * example after an upgrade that added, removed or renamed a
		 * dock, or if the saved value is corrupted/truncated). When that
		 * happens every dock reverts to its hard-coded createDockWindows()
		 * default instead of the user's last choice, even though the
		 * separately-restored main window geometry is unaffected -- see
		 * MainWindow::applySavedGeometry().
		 *
		...
		/*!
		 * Returns true if the evaluation bar should show black's
		 * colour at the bottom of the strip when the board is
		 * currently flipped (black playing from the bottom of
		 * the screen); otherwise the bar always keeps white at
		 * the bottom regardless of board orientation.
		 *
		 * Defaults to false, matching Cute Chess's historical
		 * behaviour. Configurable from the Settings dialog
		 * (General tab). Persisted in QSettings under
		 * "ui/eval_bar_black_at_bottom".
		 */
		bool evalBarBlackAtBottom() const;
	...
	public slots:
		...
		/*!
		 * Sets whether the evaluation bar should show black's
		 * colour at the bottom when the board is flipped to
		 * \a blackAtBottom, persists it, and notifies any
		 * listeners via evalBarBlackAtBottomChanged() so open
		 * evaluation bars update immediately without restarting.
		 */
		void setEvalBarBlackAtBottom(bool blackAtBottom);
	...
	signals:
		...
		/*!
		 * Emitted after setEvalBarBlackAtBottom() changes
		 * whether the evaluation bar shows black's colour at
		 * the bottom when flipped, so already-open evaluation
		 * bars can update without a restart.
		 */
		void evalBarBlackAtBottomChanged(bool blackAtBottom);
	...
	private:
		...
		QVariantMap m_pendingDockVisibility;
