...
void EvalBar::setBoardFlipped(bool flipped)
{
	if (flipped == m_boardFlipped)
		return;
...
void EvalBar::setBlackAtBottomPreference(bool enabled)
{
	if (enabled == m_blackAtBottomPref)
		return;

	m_blackAtBottomPref = enabled;
	update();
}
...
void EvalBar::paintEvent(QPaintEvent*)
{
	...
	// unless the user has opted in to matching the board's current
	// orientation (m_blackAtBottomPref) and the board is actually
	...
	if (!invert)
	{
		blackRect = QRect(0, 0, width(), h - whiteHeightPx);
		whiteRect = QRect(0, h - whiteHeightPx, width(), whiteHeightPx);
	}
	else
	{
		int blackHeightPx = h - whiteHeightPx;
		whiteRect = QRect(0, 0, width(), whiteHeightPx);
		blackRect = QRect(0, whiteHeightPx, width(), blackHeightPx);
	}
