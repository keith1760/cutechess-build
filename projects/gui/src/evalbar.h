 ...
 *
 * By default the bar always shows white's colour at the bottom of the
 * strip, regardless of which side is displayed at the bottom of the
 * board. If the user turns on the "Show black at the bottom of the
 * evaluation bar..." preference (see setBlackAtBottomPreference()),
 * the bar instead matches the board: when the board is flipped so
 * black is at the bottom (see setBoardFlipped()), the bar's colours
 * invert too, so black's colour is at the bottom whenever black's
 * pieces are.
 *
...
class EvalBar : public QWidget
{
	...
	public slots:
		...
		//! Tracks the board's current flip state (true when the
		//! black pieces are being displayed at the bottom of the
		//! screen). Combined with the "black at bottom" user
		//! preference (see setBlackAtBottomPreference()) to decide
		//! whether the bar itself should invert.
		void setBoardFlipped(bool flipped);

		//! Sets the user preference for whether the bar should
		//! show black's colour at the bottom when the board is
		//! flipped, instead of always keeping white at the
