...
void MainWindow::applySavedGeometry()
{
	...
	//
	// restoreState() bundles every dock's position *and* visibility
	// into one opaque blob, and if that blob doesn't match the current
	// set of dock widgets exactly (e.g. a dock was added, removed or
	// renamed since it was saved, or the value is corrupted/truncated)
	// it does nothing at all -- silently leaving every dock at its
	// hard-coded createDockWindows() default instead of the user's last
	// choice. restoreGeometry() just above has no such fragility, since
	// it only concerns the main window's own rectangle, so the window's
	// *position* keeps restoring correctly even when dock *visibility*
	// silently reverts to default. That split is exactly the reported
	// bug: the window (and, when restoreState() does succeed, each
	// dock) still turns up in the right place, but a dock that was
	// ticked on is nonetheless left off the screen entirely because its
	// tick was never actually restored.
	//
	// Each dock's ticked state is also saved as its own plain boolean
	// (see stageGeometryForShutdown()/dockVisibilityMap() and
